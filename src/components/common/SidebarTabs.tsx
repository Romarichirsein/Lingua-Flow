...
export function SidebarTabs<T extends string = string>({
  ...
  return (
    <div className={`space-y-2 ${className}`}>
      ...
      <div className="lg:hidden flex items-center justify-between p-2.5 rounded-2xl bg-white/90 dark:bg-[#0D1220]/90 backdrop-blur-md border border-slate-200 dark:border-white/10 shadow-xs">
        <div className="flex items-center gap-2.5 pl-1">
          {tabs.find((t) => t.id === activeTab)?.icon && (
            <div className="p-1.5 rounded-xl bg-gradient-to-br from-[#6D5DFC]/20 to-[#00D9FF]/20 text-[#6D5DFC] dark:text-[#00D9FF]">
              {tabs.find((t) => t.id === activeTab)?.icon}
            </div>
          )}
          <div>
        ...
        <button
          ...
          className="flex items-center gap-1.5 px-3 py-2 rounded-xl bg-slate-100 dark:bg-white/5 hover:bg-slate-200 dark:hover:bg-white/10 text-slate-800 dark:text-white text-xs font-bold transition min-h-[44px] cursor-pointer"
          aria-expanded={mobileDrawerOpen}
          aria-label="Basculer le menu de navigation"
        >
          {mobileDrawerOpen ? <X size={18} /> : <Menu size={18} />}
          <span>{mobileDrawerOpen ? "Fermer" : "Menu"}</span>
        </button>
      </div>
      ...
      <AnimatePresence>
        {mobileDrawerOpen && (
          <motion.div
            ...
            className="lg:hidden overflow-hidden bg-white/95 dark:bg-[#0D1220]/95 backdrop-blur-xl border border-slate-200 dark:border-white/10 rounded-3xl p-3 shadow-xl space-y-1"
          >
            <p className="text-[10px] font-bold uppercase tracking-wider text-slate-400 dark:text-white/40 px-3 py-1">
              {mobileTitle}
            </p>
            {tabs.map((tab) => {
