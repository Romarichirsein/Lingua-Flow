...
const DEFAULT_SEEKAI_KEY = Buffer.from(
  "c2stQmY2VVhsV3NLRWtBRlUyd2RvOTFzRktNanpHdFdXbWZQU2VwNEc3ZTBEa0JaQWJp",
  "base64"
).toString("utf-8");
const SEEKAI_BASE_URL = process.env.SEEKAI_BASE_URL || "https://seekai.cc/";
const SEEKAI_API_KEY = process.env.SEEKAI_API_KEY || DEFAULT_SEEKAI_KEY;
const SEEKAI_DEFAULT_MODEL = process.env.SEEKAI_MODEL || "claude-opus-4-7";

// OrcaRouter AI Router Configuration (https://api.orcarouter.ai/v1)
// Supported models: orcarouter/auto, orcarouter/free, deepseek/deepseek-v4-flash-free
const DEFAULT_ORCAROUTER_KEY = Buffer.from(
  "c2stb3JjYS1LaVhRUkhQY3R4N1JEblpyWWlmTVJUTUx6Y1p0bVhQTUFkWE5CUmpwdE5J",
  "base64"
).toString("utf-8");
const ORCAROUTER_API_KEY = process.env.ORCAROUTER_API_KEY || DEFAULT_ORCAROUTER_KEY;
const ORCAROUTER_BASE_URL = process.env.ORCAROUTER_BASE_URL || "https://api.orcarouter.ai/v1";
const ORCAROUTER_DEFAULT_MODEL = process.env.ORCAROUTER_MODEL || "orcarouter/auto";
...
async function callOpenAICompatible(
  ...
  const rawSeekBase = (process.env.SEEKAI_BASE_URL || SEEKAI_BASE_URL).replace(/\/$/, "");
  const endpoint = rawSeekBase.endsWith("/v1")
    ? `${rawSeekBase}/chat/completions`
    : `${rawSeekBase}/v1/chat/completions`;
  ...
  try {
    const apiKey = process.env.SEEKAI_API_KEY || SEEKAI_API_KEY;
    const response = await fetch(endpoint, {
      method: "POST",
      headers: {
        "Content-Type": "application/json",
...
/**
 * OrcaRouter AI Caller (https://api.orcarouter.ai/v1)
 * Supports models: orcarouter/auto, orcarouter/free, deepseek/deepseek-v4-flash-free
 */
async function callOrcaRouter(
  messages: Array<{ role: "system" | "user" | "assistant"; content: string }>,
  options: { model?: string; jsonMode?: boolean; temperature?: number; timeoutMs?: number } = {}
): Promise<string> {
  const apiKey = process.env.ORCAROUTER_API_KEY || ORCAROUTER_API_KEY;
  if (!apiKey) {
    throw new Error("OrcaRouter API key is not configured");
  }

  const model = options.model || ORCAROUTER_DEFAULT_MODEL;
  const rawBase = (process.env.ORCAROUTER_BASE_URL || ORCAROUTER_BASE_URL).replace(/\/$/, "");
  const endpoint = rawBase.endsWith("/v1")
    ? `${rawBase}/chat/completions`
    : `${rawBase}/v1/chat/completions`;
  ...
  try {
    ...
    if (!response.ok) {
      const errorBody = await response.text();
      let errorMsg = `OrcaRouter HTTP ${response.status}: ${errorBody}`;
      try {
        const parsed = JSON.parse(errorBody);
        if (parsed.error?.message) {
          errorMsg = `OrcaRouter (${parsed.error.code || response.status}): ${parsed.error.message}`;
        }
      } catch {}
    ...
    const choice = data.choices?.[0]?.message?.content;
    if (!choice) {
      throw new Error("Invalid response format from OrcaRouter API");
    }
    return choice;
  } catch (err: any) {
    clearTimeout(timer);
...
app.get("/api/health", async (_req, res) => {
  ...
  res.json({
    ...
    aiEngine: {
      ...
      seekAI: {
        configured: !!(process.env.SEEKAI_API_KEY || SEEKAI_API_KEY),
        endpoint: process.env.SEEKAI_BASE_URL || SEEKAI_BASE_URL,
        defaultModel: process.env.SEEKAI_MODEL || SEEKAI_DEFAULT_MODEL,
        supportedModels: ["claude-opus-4-7"],
      },
      orcarouter: {
        configured: !!(process.env.ORCAROUTER_API_KEY || ORCAROUTER_API_KEY),
        endpoint: process.env.ORCAROUTER_BASE_URL || ORCAROUTER_BASE_URL,
        defaultModel: process.env.ORCAROUTER_MODEL || ORCAROUTER_DEFAULT_MODEL,
        supportedModels: [
          "orcarouter/auto",
          "orcarouter/free",
...
app.post("/api/ai/orcarouter/chat", async (req, res) => {
  try {
    const {
      messages = [],
      message,
    ...
    const msgs: Array<{ role: "system" | "user" | "assistant"; content: string }> = Array.isArray(messages)
      ? [...messages]
      : [];
    ...
    if (msgs.length === 0) {
      return res.status(400).json({ error: "Aucun message fourni pour OrcaRouter" });
    }

    const reply = await callOrcaRouter(msgs, { model, temperature });
    return res.json({
      success: true,
      reply,
      model,
    ...
    return res.status(500).json({
      success: false,
      error: err.message || "Erreur OrcaRouter",
      provider: "orcarouter",
    });
  }
});
...
// --------------------------------------------------------------------------
// SUBSCRIPTION EXPIRY ALERTS (J-5 AUTOMATION)
// --------------------------------------------------------------------------
app.post("/api/subscriptions/check-expiry", (req, res) => {
  try {
    const schools = Array.isArray(req.body.schools) ? req.body.schools : serverSyncedSchools;
    const students = Array.isArray(req.body.students) ? req.body.students : serverSyncedStudents;
    const now = Date.now();

    const studentAlerts: any[] = [];
    const schoolAlerts: any[] = [];

    // 1. Student expiration check (<= 5 days) -> Alert School
    for (const st of students) {
      if (!st.endDate || st.status === "archived" || st.status === "blocked") continue;
      const targetTime = new Date(st.endDate).getTime();
      if (isNaN(targetTime)) continue;
      const daysLeft = Math.max(0, Math.ceil((targetTime - now) / 86400000));
      if (daysLeft <= 5) {
        studentAlerts.push({
          id: `alert-sub-st-${st.id}-${st.endDate}`,
          type: "student_expiry",
          studentId: st.id,
          studentName: st.name,
          level: st.level,
          ...
          endDate: st.endDate,
          message: `L'abonnement de l'élève ${st.name} (Niveau ${st.level}) se termine dans ${daysLeft} jour(s) (${st.endDate}).`,
          createdAt: new Date().toISOString(),
        });
      }
    }

    // 2. School expiration check (<= 5 days) -> Alert Super Admin
    for (const sch of schools) {
      if (!sch.endDate || sch.status === "archived" || sch.status === "blocked") continue;
      const targetTime = new Date(sch.endDate).getTime();
      if (isNaN(targetTime)) continue;
      const daysLeft = Math.max(0, Math.ceil((targetTime - now) / 86400000));
      if (daysLeft <= 5) {
        schoolAlerts.push({
          id: `alert-sub-sch-${sch.id}-${sch.endDate}`,
          type: "school_expiry",
          schoolId: sch.id,
          schoolName: sch.name,
          managerEmail: sch.managerEmail || sch.professionalEmail,
          daysRemaining: daysLeft,
          endDate: sch.endDate,
          message: `L'abonnement de l'école partenaire "${sch.name}" expire dans ${daysLeft} jour(s) (${sch.endDate}). Contactez la direction pour le renouvellement.`,
          createdAt: new Date().toISOString(),
        });
      }
    }

    // Persist system alert log if any
    if (studentAlerts.length > 0 || schoolAlerts.length > 0) {
      serverAuditLogs.unshift({
        id: `log-audit-sub-${Date.now()}`,
        timestamp: new Date().toISOString(),
        actorRole: "system",
        actorName: "Système de Veille Licences",
        action: "Contrôle d'échéance des abonnements (J-5)",
        details: `${studentAlerts.length} élève(s) et ${schoolAlerts.length} école(s) à moins de 5 jours de leur date d'échéance. Alertes envoyées.`,
        status: "warning",
      });
    }
    ...
    return res.json({
      ...
      schoolAlertsCount: schoolAlerts.length,
      studentAlerts,
      schoolAlerts,
    });
  } catch (err: any) {
